...
\cugthesisteacher{蔡之华\quad 教授}{Zhihua Cai}
\cugthesisteachertwo{测试}{ceshi}
\cugthesisauthor{王震宇}{Zhenyu Wang}
\educatingunit{计算机学院}
\cugthesisdate{2018}{3}
\cugclassnumber{P5}          % 分类号（封面用，规范 1.1）
\cugsecretlevel{公开}        % 密级（封面用，规范 1.1 / 2.2）
...
% 学位论文答辩委员会名单（规范 1.7 / 附件 7）
% \cugcommitteemember{职务}{姓名}{职称}{单位}
% 会由 \makefrontpages 自动排在中文摘要之前；尚未答辩时可先注释掉。
\cugcommitteemember{主席}{李某某}{教授}{中国地质大学（武汉）}
\cugcommitteemember{委员}{张某某}{教授}{武汉大学}
\cugcommitteemember{委员}{王某某}{教授}{中国地质大学（武汉）}
\cugcommitteemember{委员}{陈某某}{副教授}{中国地质大学（武汉）}
\cugcommitteemember{秘书}{赵某某}{讲师}{中国地质大学（武汉）}
...
\begin{tfig}{Overleaf 注册账号1}{fig:overleaf_usage_1}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/2.png}
\end{tfig}
...
\begin{tfig}{Overleaf 注册账号2}{fig:overleaf_usage_2}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/3.png}
\end{tfig}
...
\begin{tfig}{Overleaf  创建项目1}{fig:overleaf_usage_3}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/5.png}
\end{tfig}
...
\begin{tfig}{Overleaf  创建项目2}{fig:overleaf_usage_4}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/6.png}
\end{tfig}

\begin{tfig}{Overleaf  创建项目3}{fig:overleaf_usage_5}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/7.png}
\end{tfig}

点击打开, 进入上传页面, 如图\ref{fig:overleaf_usage_6}所示, 静静等待上传完毕.

\begin{tfig}{Overleaf  创建项目4}{fig:overleaf_usage_6}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/8.png}
\end{tfig}
...
\begin{tfig}{Overleaf   项目设置1}{fig:overleaf_usage_7}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/9.png}
\end{tfig}

\begin{tfig}{Overleaf  项目设置 2}{fig:overleaf_usage_8}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/10.png}
\end{tfig}
...
\begin{tfig}{Overleaf   编译}{fig:overleaf_usage_9}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/11.png}
\end{tfig}

静静等待一段时间后, 编译成功, 如图\ref{fig:overleaf_usage_10}所示.

\begin{tfig}{Overleaf   编译结果}{fig:overleaf_usage_10}
    \cugdemoimage{width=.5\textwidth}{./imgs/overleaf_usage/12.png}
\end{tfig}
...
\begin{ttab}{论文相关信息命令}{tab:cugthesisinfocmds}
    \begin{tabular}{lll}
        ...
        命令 & 作用 & 用法 \\
        \midrule
        \tcodeinline{tex}{\cugthesistitle} & 论文标题 & \tcodeinline{tex}{\cugthesistitle{cn}{en}} \\
        \tcodeinline{tex}{\cugthesismajor} & 专业 & \tcodeinline{tex}{\cugthesismajor{cn}{en}} \\
        \tcodeinline{tex}{\cugthesisteacher} & 指导老师 & \tcodeinline{tex}{\cugthesisteacher{cn}{en}} \\
        \tcodeinline{tex}{\cugthesisauthor} &  作者姓名 & \tcodeinline{tex}{\cugthesisauthor{cn}{en}} \\
...
\begin{ttab}{摘要相关命令}{tab:abstractcmds}
    \begin{tabular}{ll}
        ...
        命令 & 用法 \\
        \midrule
        \tcodeinline{tex}{\cugabstract} & \tcodeinline{tex}{\cugabstract{中文摘要}{Abstract}} \\
...
\section{Versino 0.5：依 2025 版规范全面校准}
\label{sec:versino_0_5}

2026-09

学校于 2025 年 5 月修订了《研究生学位论文写作规范》，本版据此逐条校准，
并对照附件 1--12 的实物样式复核。主要改动：

\begin{itemize}
    \item 字体：显式指定宋体 / 黑体 / Times New Roman，不再依赖编译环境自动探测
          （旧版在 Windows 与 Overleaf 上渲染结果不同）
    \item 正文行距改为固定值 20 磅；摘要标题 18 磅；图表题注 10.5 磅
    \item 页眉位置、目录缩进、图表清单名称等按规范逐条修正
    \item 新增中文封面、本科封面、学位论文答辩委员会名单
    \item 声明类页面（原创性声明 / 导师承诺书 / 使用授权书）依附件实测改为
          四号 14 磅配 2 / 2 / 1.5 倍行距，落款缩进亦按附件取值
    \item 补上「同等学力博士 / 硕士」两种学位类型（规范 2.4 的页眉文字共六种）
\end{itemize}

逐项对照表见 README 的「格式说明」一节。

\backmatter{}
% 规范 1.13 / 1.14 / 1.15 的顺序是：参考文献 -> 致谢 -> 附录。
% 旧版示例把致谢写在参考文献之前，容易被直接照抄，此处修正。
\cugthesisbib{tex}
